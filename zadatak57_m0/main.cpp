...
void signalProcessing(double pIn[][BLOCK_SIZE], double pOut[][BLOCK_SIZE], double *variableGains, int nSamples)
{
	double temp;
	...
	for (int i = 0; i < BLOCK_SIZE; i++)
	{
		...
		pIn[L_CH][i] = pIn[L_CH][i] * variableGains[L_CH];
		pIn[R_CH][i] = pIn[R_CH][i] * variableGains[R_CH];

		// OBRADA IZ LEVOG KANALA

		// druga faza, primenjujemo LPF 11kHz i rezultat saljemo na izlaz Ls
		pOut[LS_CH][i] = saturation(iir_l_lpf11(pIn[L_CH][i]), limiterThreshold);

		// treca faza, primenjujemo HPF 5kHz, smanjujemo za -10dB i rezultat saljemo na izlaz C
		pOut[C_CH][i] = iir_l_hpf5(pIn[L_CH][i]);
		temp = pOut[C_CH][i] * MINUS_10DB;
		pOut[C_CH][i] = saturation(temp, limiterThreshold);

		// cetvrta faza, primenjujemo HPF 3kHz, smanjujemo za -4dB, sabiramo sa C izlaznim kanalom i saljemo na izlaz L
		pOut[L_CH][i] = iir_l_hpf3(pOut[LS_CH][i]);
		pOut[L_CH][i] = pOut[L_CH][i] * MINUS_4DB;
		pOut[L_CH][i] = saturation(pOut[L_CH][i] + temp, limiterThreshold);

		// OBRADA IZ DESNOG KANALA

		// peta faza, primenjujemo HPF 5kHz
		pOut[RS_CH][i] = saturation(iir_r_lpf11(pIn[R_CH][i]), limiterThreshold);

		// sesta faza, primenjujemo HPF 5kHz, smanjujemo za -9.5dB i smestamo u temp
		temp = iir_r_hpf5(pIn[R_CH][i]);
		temp = temp * MINUS_9P5DB;

		// sedma faza, primenjujemo HPF 3kHz, smanjujemo za -3.9dB, sabiramo sa temp i saljemo na izlaz R
		pOut[R_CH][i] = iir_r_hpf3(pOut[RS_CH][i]);
		pOut[R_CH][i] = pOut[R_CH][i] * MINUS_3P9DB;
		pOut[R_CH][i] = saturation(pOut[R_CH][i] + temp, limiterThreshold);
	}
}
...
int main(int argc, char *argv[])
{
	...
	//-------------------------------------------------
	{
		...
		for (int i = 0; i < iNumSamples / BLOCK_SIZE; i++)
		{
			...
			if (enabled)
				signalProcessing(sampleBuffer, sampleBuffer, variablesGain, BLOCK_SIZE);
